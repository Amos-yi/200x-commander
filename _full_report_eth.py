...
STATE_FILE = r"C:\Users\Administrator\200x_commander\rt_paper_v2_state_eth.json"
...
for key, d in data.items():
    ...
    rows.append([key, eq, eq - 100, tc, tp2_ct, sl_ct, rev_ct, wr, pf, avg_r, peak, dd_pct, pos_side])
...
print("=== ETH_USDT 5m Full Report ===")
print(f"{'#':>3s} {'Strategy':<26s} {'Eq':>8s} {'PnL%':>7s} {'Trade':>5s} {'TP2':>4s} {'SL':>4s} {'Rev':>4s} {'Win%':>5s} {'PF':>6s} {'|R|':>5s} {'DD%':>5s}")
print("-" * 128)
for i, r in enumerate(rows[:30], 1):
    n, eq, pnl, tc, tp2, sl, rev, wr, pf, ar, pk, dd, ps = r
    print(f"{i:>3d} {n:<26s} {eq:>8.2f} {pnl:>+6.1f}% {tc:>5d} {tp2:>4d} {sl:>4d} {rev:>4d} {wr:>4.0f}% {pf:>6.2f} {ar:>5.2f} {dd:>4.1f}%")

print(f"\nTOTAL: {total_eq:.0f}U | Trades:{total_trades} | TP2:{total_tp2} SL:{total_sl} | {len(rows)} strategies\n")

bins = {"+100%": 0, "+50~100%": 0, "+0~50%": 0, "-0~-15%": 0, "-15%以下": 0, "未交易": 0}
for r in rows:
    pct = r[2]; tc = r[3]
    if tc == 0: bins["未交易"] += 1
    elif pct >= 100: bins["+100%"] += 1
    elif pct >= 50: bins["+50~100%"] += 1
    elif pct >= 0: bins["+0~50%"] += 1
    elif pct >= -15: bins["-0~-15%"] += 1
    else: bins["-15%以下"] += 1
for bk, ct in bins.items(): print(f"  {bk:<10s} {ct:>3d}  {'█'*ct}")
