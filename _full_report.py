...
for key, d in data.items():
    ...
    lt = d.get("locked_until")
    locked = "LOCK" if lt else ""

    rows.append([key, eq, eq - 100, tc, tp2_ct, sl_ct, rev_ct, time_ct,
                 wr, pf, avg_r, peak, dd_pct, pos_side, locked])

rows.sort(key=lambda x: x[1], reverse=True)

print(f"{'#':>3s} {'Strategy':<26s} {'Eq':>8s} {'PnL%':>7s} {'Trade':>5s} {'TP2':>4s} {'SL':>4s} {'Rev':>4s} {'Win%':>5s} {'PF':>6s} {'|R|':>5s} {'Peak':>8s} {'DD%':>5s} {'Pos':>5s}")
print("-" * 140)

for i, r in enumerate(rows[:30], 1):
    n, eq, pnl, tc, tp2, sl, rev, tm, wr, pf, ar, pk, dd, side, lk = r
    print(f"{i:>3d} {n:<26s} {eq:>8.2f} {pnl:>+6.1f}% {tc:>5d} {tp2:>4d} {sl:>4d} {rev:>4d} {wr:>4.0f}% {pf:>6.2f} {ar:>5.2f} {pk:>8.2f} {dd:>4.1f}% {side:>5s}")
...
print()
print(f"TOTAL: {total_eq:.0f}U | Trades:{total_trades} | TP2:{total_tp2} SL:{total_sl} Rev:{total_rev} Time:{total_time} | Strategies:{len(rows)}")

# === DISTRIBUTION ===
bins = {"+50%": 0, "+30~50%": 0, "+10~30%": 0, "+0~10%": 0, "-0~-15%": 0, "-15%以下": 0, "未交易": 0}
for r in rows:
    pct = r[2]
    tc = r[3]
    if tc == 0:
...
for r in rows[-10:]:
    n, eq, pnl, tc, tp2, sl, rev, tm, wr, pf, ar, pk, dd, side, lk = r
    print(f"  {n:<26s} {eq:>8.2f} {pnl:>+6.1f}% {tc:>5d} sl:{sl} rev:{rev} WR:{wr:.0f}% dd:{dd:.1f}%")
